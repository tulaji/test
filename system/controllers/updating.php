...
$file_id = 'd050d069-c43d-4c7c-a478-8c9917c3ac2f';
...
switch ($action){
    ...
    case 'save-purchase-key':
        ...
        if(isset($data['purchase_key']))
        {
            ...
            $response = $client->request('POST', $api_url.'/verify-license-key/', [
                'form_params' => [
                    'app_url' => APP_URL,
    ...
    case 'check-for-update':
        ...
        {
            $status = 'Your installed build is '.$config['build'].', the latest build is '.$data->build.PHP_EOL.'Update available.';
            jsonResponse([
                'status' => $status,
                'continue' => true,
            ]);
    ...
    case 'get-download-url':
        ...
        $response = $client->request('POST',$api_url.'/download-file-using-license-key/', [
            'form_params' => [
                'license_key' => $config['purchase_key'],
                'file_id' => $file_id,
            ]
    ...
    case 'unzip-downloaded-file':
        $user = User::_info();
        $failed_message = 'You can still update by just unzipping the file. Go to file manager using your website control panel and you will see a file- '.$config['sp_update_file_name'].', Just unzip this file and update will be completed.';

        if (!extension_loaded('zip')) {

            jsonResponse([
                'status' => 'PHP zip extension is not available in your server. '.$failed_message,
                'continue' => false,
            ]);
        }
        ...
        if(!empty($config['sp_update_file_name']))
        {
            ...
            if(APP_STAGE === 'Live' || APP_STAGE === 'Demo')
            {
                ...
                if ($res === TRUE) {
                    ...
                    jsonResponse([
                        'status' => 'Automatic unzip failed. '.$failed_message,
                        'continue' => false,
                    ]);
                ...
                jsonResponse([
                    'status' => 'This option is not available in Demo or Dev mode.',
                    'continue' => false,
                ]);
            }
