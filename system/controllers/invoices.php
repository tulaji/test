    ...
    case 'add-post':
        ...
        if ($msg == '') {
            ...
            foreach ($amount as $samount) {
                ...
                //      $total_linetds = ($line_total+$lTaxVal) * ($tds_deduction / 100);
                //      $d->tds_value = $total_linetds;
                //      $d->tds_rate = $ua->tds_deduction;

                //   $d->total = $line_total-$total_linetds;
                //   $d->total_notds=$line_total;
                // }else{
                //       $d->total = $line_total;
                //       $d->total_notds=$line_total;
                // }
               ...
               if($_POST['is_tds'][$i]=='YES'){
                     $ua = ORM::for_table('crm_accounts')->find_one($cid);
                     $tds_deduction= $ua->tds_deduction;
                     $total_tds = $total_tds+(($line_total+$lTaxVal) * ($tds_deduction / 100));
...
if (_post('sameasbilling')=='0') {

$shipping_addresses =ORM::for_table('shipping_addresses')->create();
$shipping_addresses->address_line_1=_post('m_address_shipping');
$shipping_addresses->contact_id=$cid;
$shipping_addresses->city=_post('city_shipping');
$shipping_addresses->state=_post('state_shipping');
$shipping_addresses->zip=_post('zip_shipping');
$shipping_addresses->country=_post('country_shipping');
$shipping_addresses->save();
$shippingid = $shipping_addresses->id();


  $invoice->shipping_street = _post('m_address_shipping');
   $invoice->shipping_city = _post('city_shipping');
    $invoice->shipping_state = _post('state_shipping');
     $invoice->shipping_zip = _post('zip_shipping');
     $invoice->shipping_country = _post('country_shipping');
   ...
   //   $invoice->shipping_country = _post('country_shipping');
     ...
     $shippingid = '0';
          ...
          $invoice->shippingid = $shippingid;
            $invoice->original_for_buyer = _post('original_for_buyer');
            $invoice->duplicate_for_transporter = _post('duplicate_for_transporter');
            $invoice->third_copy_supplier = _post('third_copy_supplier');
            $invoice->bank_account_details = _post('bank_account_details');

             $invoice->declaration = _post('declaration');

            
            $invoice->tds_value=$total_tds;
            ...
            $invoice->total = $fTotal-$total_tds;
            ...
            if ($invoice) {
                ...
                Goodsinventory::where('invoice_id', $invoice->id)->delete();
            }
            ...
            foreach ($description as $item) {
                ...
                $is_tds = $_POST['is_tds'][$i];


                $d = ORM::for_table('sys_invoiceitems')->create();
                $d->invoiceid = $invoiceid;
                $d->tds_deduction= $is_tds;
                $d->userid = $cid;
                $d->description = $item;
                $d->qty = $sqty;
                $d->amount = $samount;
                $d->discount_rate=$line_discount;
                ...
                $d->discount_amount = $actual_line_discount;
                ...
                $raw_inventory->ref_no =$item_number[$i];
                $raw_inventory->item_id =$item_number[$i];
                $raw_inventory->qty =$sqty;
                $raw_inventory->invoice_id=$invoiceid;
    ...
    case 'list':
        ...
        $partially_paid = ORM::for_table('sys_invoices')->where(
            'status',
            'Partially Paid'
        );
        ...
        $cancelled = ORM::for_table('sys_invoices')->where(
            'status',
            'Cancelled'
        );
