...
switch ($action) {
    case 'add':
        ...
        $ui->assign(
            'xfooter',
            Asset::js(['modal', 's2/js/select2.min', 's2/js/i18n/' . lan()])
        );
    ...
    case 'add-post':
        ...
        if ($company_id != '') {
            ...
            $c->phone = $phone;
             ...
             $c->company_type =_post('company_type');
             $c->tds_deduction = _post('tds_deduction');
        ...
        if (_post('company_type') == '') {
                    $msg .= 'Company Type is required' . ' <br>';
                }
        if (_post('tds_deduction') == '') {
            $msg .= 'TDS rate is required'. ' <br>';
        }
        ...
        if ($msg == '') {
            ...
            $d->tags = Arr::arr_to_str($tags);
             ...
             $d->company_type =_post('company_type');
...
";
    ...
    case 'modal_add':
        ...
        break;
         ...
         case 'modal_add_shipping':
        Event::trigger('contacts/modal_add_shipping/');

        $ui->assign('countries', Countries::all($config['country'])); 
        // 

         // $shipping_addresses = ShippingAddress::where('contact_id', '8')
         //    ->get();

         // $ui->assign('shipping_addresses',$shipping_addresses); 

        // may add this $config['country_code']

        view('modal_add_shipping');
...
'
    ...
    case 'modal_add_company':
        ...
        if ($company) {
            ...
            $val['country'] = $company->country;
             ...
             $val['company_type'] =$company->company_type;
             $val['tds_deduction'] =$company->tds_deduction;
    ...
    case 'add_company_post':
        ...
        if ($data['company_type'] == '') {
            i_close('Company Type is required');
        }

         if ($data['tds_deduction'] == '') {
            i_close('TDS Deduction is required');
        }
        ...
        $company->tds_deduction = $data['tds_deduction'];
